...
final class HealthReportService
{
     ...
     * 
     * @param Healthjournal $journal The journal to generate report for
     * @param User|null $user The current user for ownership validation
     * @throws \AccessDeniedException If user doesn't own the journal
     */
    public function generateReport(Healthjournal $journal, ?User $user = null): HealthReportDTO
    {
        // Validate user ownership
        if ($user && $journal->getUser() !== $user) {
            throw new \InvalidArgumentException('Vous n\'avez pas accès à ce journal');
        }
        
        // Get entries from the journal's date range (not just last 30 days)
        $startDate = $journal->getDatedebut();
        $endDate = $journal->getDatefin() ?? new \DateTime();
        
        // If no start date, fall back to getting all entries for the journal
        if ($startDate === null) {
            $entries = $this->entryRepository->findBy(['journal' => $journal], ['date' => 'DESC']);
        } else {
            $entries = $this->entryRepository->findByJournalAndDateRange($journal, $startDate, $endDate);
        }
        ...
        // Get prediction
        $prediction = $this->predictionService->predictGlycemia($journal, $user);
     ...
     * 
     * @param User|null $user The current user for ownership verification
     */
    public function getJournal(?int $journalId, ?User $user = null): ?Healthjournal
    {
        if ($journalId === null) {
            return $this->getDefaultJournal($user);
        }
        
        $journal = $this->journalRepository->find($journalId);
        
        // Verify ownership if user is provided
        if ($journal && $user && $journal->getUser() !== $user) {
     ...
     */
    public function getAllJournals(?User $user = null): array
    {
        if ($user === null) {
        ...
        return $this->journalRepository->findBy(['user' => $user], ['datedebut' => 'DESC']);
    }
     ...
     */
    public function getJournalsWithEnoughEntries(?User $user = null): array
    {
        if ($user === null) {
            $user = $this->security->getUser();
        }
     ...
     * @param User|null $user The current user
     */
    private function getDefaultJournal(?User $user = null): ?Healthjournal
    {
        if ($user === null) {
            $user = $this->security->getUser();
        }
    ...
    private function getPatientName(Healthjournal $journal): string
    {
        ...
        return $journal->getName() ?? 'Patient';
    }
