...
class NutritionController extends AbstractController
{
    ...
    /**
     * Get the current user's UUID string
     */
    ...
    public function addFood(Request $request, FoodLogRepository $foodLogRepository): Response
    {
        ...
        if ($quickName && $quickCalories > 0) {
            ...
            if ($request->isXmlHttpRequest()) {
                ...
                $foodLogs = $foodLogRepository->findAllByUserUuidAndDate($userId, $today);
    ...
    public function progress(
        ...
        for ($i = 6; $i >= 0; $i--) {
            ...
            $foodLogs = $foodLogRepository->findAllByUserUuidAndDate($userId, $date);
...
return $this->render('nutrition/nutrition-analysis.html.twig', [
     ...
     * Save conversation to database for persistence
     */
    private function saveConversation(string $userId, string $userMessage, string $aiResponse, ?array $goalData = null): void
    {
        ...
        if ($mealData && $user) {
            ...
            if (is_array($meals)) {
                ...
                foreach ($meals as $meal) {
                    ...
                    // Set the user and user ID so the meal appears in the user's planner
    ...
    {
        $userId = $this->getUserId();
        // Convert UUID to integer for database query
        $userIdInt = is_string($userId) ? (int) hexdec(substr($userId, 0, 8)) : $userId;
        $conversation = $conversationRepository->findOneBy(['id' => $id, 'userId' => $userIdInt]);
    ...
    #[Route('/ai-clear-history', name: 'nutrition_ai_clear_history')]
    public function aiClearHistory(Request $request, AiConversationRepository $conversationRepository): Response
    {
        $userId = $this->getUserId();
        
        // Clear database conversations - repository now handles UUID conversion internally
